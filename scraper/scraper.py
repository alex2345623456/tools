...
# Fungsi untuk setup WebDriver dengan profil Chrome yang sudah login
def setup_driver_with_profile():
    # Path ke direktori profil Chrome yang sudah login
    profile_path = "/home/alex/.config/google-chrome"  # Ganti dengan path yang sesuai
    
    options = webdriver.ChromeOptions()
    options.add_argument(f"user-data-dir={profile_path}")  # Menggunakan direktori profil Chrome
    options.add_argument("profile-directory=Default")  # Menggunakan profil 'Default' (ganti jika profil lain digunakan)
    
    # Menambahkan pengaturan untuk mode headless
    # options.add_argument("--headless")  # Mode headless, tidak menampilkan browser
...
def fetch_page_source(driver, url, output_file):
    driver.get(url)
    time.sleep(random.uniform(1, 2))  # Tidur acak antara 1 hingga 2 detik
...
def main():
    ...
    driver = setup_driver_with_profile()  # Menggunakan profil Chrome yang sudah login
    ...
    try:
        ...
        for batch in batches:
            ...
            for url in urls:
                ...
                time.sleep(random.uniform(1, 3))  # Tidur acak

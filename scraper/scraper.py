...
# Fungsi untuk menambahkan cookie dari file teks
def load_cookies_from_file(driver, cookie_file):
    with open(cookie_file, 'r') as file:
        cookies = file.read().strip()  # Membaca cookie sebagai satu string panjang
    
    # Set cookie ke dalam WebDriver
    driver.get("https://github.com")  # Akses GitHub untuk memulai sesi
    driver.add_cookie({"name": "cookie", "value": cookies})  # Menambahkan cookie langsung
    driver.refresh()  # Muat ulang halaman untuk memanfaatkan cookie yang telah ditambahkan

# Fungsi untuk login ke GitHub (Jika perlu fallback)
...
def main():
    ...
    cookie_file = "cookies.txt"  # File cookie yang selalu digunakan
    
    driver = setup_driver()

    try:
        # Langkah 1: Memuat cookie dari file cookies.txt
        print(f"Memuat cookie dari file: {cookie_file}")

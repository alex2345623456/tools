...
def generate_url_encoded_variations(body, payload):
    """Generate variations for URL-encoded body by appending payload to the value."""
    params = body.split("&")
    payloads = []
    for i, param in enumerate(params):
        key = param.split("=")[0]
        value = param.split("=")[1]
        # Append the payload to the existing value
        modified_params = params.copy()
        modified_params[i] = f"{key}={value}{payload}"
        payloads.append("&".join(modified_params))
    return payloads

def generate_json_variations(json_obj, payload):
    """Generate variations for JSON body by appending payload to existing values."""
    variations = []
...
def send_request(url, method, headers, body):
    ...
    time.sleep(random.uniform(5, 30))  # Introduce random sleep
